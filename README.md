...
| `add_coded_workflow` | Adds a coded workflow (`.cs`, inherits `CodedWorkflow`, `[Workflow]`, registered in `project.json` `entryPoints`), a coded test case (`kind=test`, `[TestCase]`, registered in `designOptions.fileInfoCollection` — never `entryPoints`), or a plain coded source file (`kind=source`). Optional `relativeFolder` (e.g. `Tests`, `Models`); Process + `kind=test` defaults to `Tests\`. Empty `relativeFolder` forces the project root. |
...
The `tests/` folder contains four xUnit projects:
