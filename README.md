...
- `McpServer:ToolSurface=ReadOnly` advertises tools annotated `ReadOnly=true` and `Destructive=false`
...
- **Recommended tools (default connector):** `analyze_project`, `search_codebase`, `read_workflow_file`, `explain_workflow`, `get_workflow_dependencies`, `validate_project`, `analyze_project_gaps`, `check_work`, `get_job`, `navigate_code`, `create_implementation_plan`, `get_implementation_plan`, `update_plan_task`, `add_coded_workflow`, `edit_workflow_file`, `find_activity`, `insert_activities`, `validate_activity_spec`, `build_workflow`, `manage_workflow_data`, `search_knowledge`, `manage_project_content`, `checkpoint`, `get_changes`, `revert_changes`, `update_canvas_snapshot`
- **Leave off the default connector:** `CopilotConnectorTools.LeaveOffNames` — hatches, aliases, and tools reserved for Inspector: `generate_documentation`, `create_project`, `add_xaml_workflow`, `patch_project_json`, `get_analyzer_rules`, `write_workflow_file`, `edit_workflow_activity`, `compile_project`, `verify_work`, `run_ui_path_cli`, `list_skills`, `search_repository`, `create_work_items`, `run_workflow`, `control_debug_session`, `get_compile_errors`, `recommend_activities`, `manage_packages`, `get_object_repository`, `get_activity_metadata`. Notable overlaps: `compile_project` (use `validate_project(build:true)`), `verify_work` / `get_compile_errors` (prefer `check_work` then `update_plan_task`), `edit_workflow_activity` (prefer `insert_activities`), `write_workflow_file` (full-file overwrite on `ToolSurface=All` only), `list_skills` (prefer `search_knowledge(mode=skill)`). `run_workflow` and `control_debug_session` execute automation and stay off the default connector (also fail-closed unless `UiPathCli:EnableExecution` is `true`). HTTP `McpServer:ToolSurface` defaults to `CopilotDefault`. Set `ReadOnly` for annotation-gated read tools only, or `All` for Inspector. GitLab tools stay registered on the server.
...
> ✅ **Green gate:** `check_work` → `update_plan_task`. Do not use `verify_work` as the done gate. Long CLI tools such as `validate_project` return `{ jobId, status: "running" }`; poll `get_job`.
...
is on the Copilot default connector for that shell. Spec grammar:
MCP resource `uipath://authoring/activity-spec`.
...
analyze_project (summary) → get_implementation_plan
   ...
   → check_work (compile + validate + scoped gaps)
...
> implement the next pending task, run `check_work` on the files you touched,
> remediate resilience/boundary gaps, then mark the task done with `update_plan_task`.
...
- `insert_activities` — Inserts a JSON activity spec as children of the activity located by `activityId` or `DisplayName`. Spec grammar: `uipath://authoring/activity-spec`.
- `validate_activity_spec` — Dry-run validation of a JSON activity spec against the catalog — no files read or written.
...
- `search_knowledge` — Knowledge lookup: `mode=activity_docs` (package doc excerpts), `mode=knowledge` (guides + activity corpus; `kind=all|guides|activityDocs`), `mode=skill` (read one skill file).
- `manage_project_content` — Docs and files: `list_docs`/`write_docs`/`delete_docs`/`search_docs`, `write_file`/`edit_file`/`delete_file`, `sync_context`, `validate_docs`. Refuses `project.json`, plan files, `docs/knowledge`/`docs/adr` via file actions, secret names, and `***REDACTED***`.
...
- `get_analyzer_rules` — Lists enabled Workflow Analyzer rules as a background job (always pass a scope); poll `get_job`.
- `write_workflow_file` — Full-file overwrite hatch. Prefer `edit_workflow_file` or `insert_activities`.
- `edit_workflow_activity` — Raw XAML fragment hatch. Prefer `insert_activities`.
- `compile_project` — CLI build alias (async job). Prefer `validate_project(build:true)` or `check_work`.
- `verify_work` — Bundled check. Prefer `check_work` then `update_plan_task`.
- `run_ui_path_cli` — Allowlisted `uip` CLI hatch.
- `list_skills` — Lists RPA playbooks. Prefer `search_knowledge(mode=skill)`.
- `search_repository` / `create_work_items` — GitLab issues.
- `run_workflow` / `control_debug_session` — Execution (also require `UiPathCli:EnableExecution=true`); async jobs → `get_job`.
- `get_compile_errors` — Structured Roslyn diagnostics without a build. Prefer `check_work` on the default connector.
- `recommend_activities` — Up to 5 version-aware activity schemas for a natural-language query.
- `manage_packages` — Canonical NuGet dependency path (`install` is an async job). Do not hand-edit `project.json` dependencies.
- `get_object_repository` — Reads the project's Object Repository (apps → screens → elements).
- `get_activity_metadata` — Full authoring surface of one named activity.
...
- `generate_documentation` — Leave-off. See Leave-off section.
- `get_activity_metadata` — Leave-off. See Leave-off section.
- `search_knowledge` — See Copilot default.

### 💻 Author (coded)

- `add_coded_workflow` — See Copilot default.
- `edit_workflow_file` — See Copilot default.
- `get_compile_errors` — Leave-off. Prefer `check_work`.
- `navigate_code` — See Copilot default.
...
- `recommend_activities` — Leave-off.
- `build_workflow` — See Copilot default.
- `manage_workflow_data` — See Copilot default.
- `add_xaml_workflow` — Leave-off. Prefer coded for new work.
- `write_workflow_file` — Leave-off full-file overwrite (`ToolSurface=All` only). Prefer `edit_workflow_file` / `insert_activities`.
- `edit_workflow_activity` — Leave-off XAML fragment hatch (`ToolSurface=All` only). Prefer `insert_activities`.
...
- `check_work` — See Copilot default. Green gate: then `update_plan_task`.
- `get_job` — See Copilot default.
- `checkpoint` / `get_changes` / `revert_changes` — See Copilot default.
- `validate_project` — See Copilot default (async job). Prefer `check_work` for the green gate.
- `verify_work` — Leave-off. Prefer `check_work` then `update_plan_task`.
- `compile_project` — Leave-off. Prefer `validate_project(build:true)`.
...
- `create_project` — Leave-off. Scaffolds via `uip rpa init`.
- `run_ui_path_cli` — Leave-off unbounded CLI hatch (`ToolSurface=All` only). Runs an allowlisted `uip` command; mutating subcommands are blocked unless enabled in config.
- `list_skills` — Leave-off. Prefer `search_knowledge(mode=skill)`.
- `search_knowledge` — See Copilot default (includes skill reads).
...
**Roslyn `analysisMode`.** The C# analysis tools (`navigate_code`, `get_compile_errors`) build a cached in-memory Roslyn compilation per
