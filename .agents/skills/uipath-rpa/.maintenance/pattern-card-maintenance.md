...
4. Gate: per-file `uip rpa validate --file-path "<RELATIVE_FILE>" --project-dir "<DIR>" --output json` to 0 errors, then `uip rpa build "<DIR>" --output json` clean. `validate` clean is NOT sufficient — it covers only the file it targets, while `build` compiles every workflow in the project.
