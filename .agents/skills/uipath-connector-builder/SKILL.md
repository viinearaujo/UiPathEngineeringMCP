...
4. **`activity create` writes both sides in one call** — the standard-resource file AND one `element.json` entry per method. Re-running on an existing activity appends/merges. Pass `--skip-sr` (or use `auth system create`) only for system resources that need no SR file. Model `GETBYID`/`PATCH`/`PUT`/`DELETE` only for TRUE by-id endpoints — never for search. The by-id `/{id}` path param is added automatically: ALWAYS for `GETBYID`, and for `PATCH`/`PUT`/`DELETE` only when the activity is CRUD (it also has a `GET`/`GETBYID`); a write-only/action activity keeps its base path. **The by-id path token and the primary key are separate** — the token is substituted before the request leaves us (the vendor never sees its name), while the primary key names a field read back from the vendor's response. A trailing `{token}` in `--vendor-path` is NOT adopted as the key; pass `--primary-key <vendor response key>`.
...
# 3. Author the field schema — REQUIRED, not optional (Rule 12). `activity create`
...
5. Modeling `GETBYID` for a search/list endpoint (Rule 4) — by-id verbs auto-add the `/{id}` param; reserve them for true single-record reads.
