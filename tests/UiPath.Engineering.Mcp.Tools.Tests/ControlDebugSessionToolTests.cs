...
public class ControlDebugSessionToolTests {
    private static (ControlDebugSessionTool Sut, RecordingStructuredCli Cli) CreateSut(
        ...
        return (new ControlDebugSessionTool(cli, filesystem, CliToolFixtures.Policy(execution: execution)), cli);
    }

    [Fact]
    public async Task ExecutionDisabledByDefault_RefusesStartWithoutRunningAnything() {
        var (sut, cli) = CreateSut(execution: false);

        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "start", filePath: "Main.xaml");
    ...
    public async Task ExecutionDisabledByDefault_RefusesReadOnlyLookingStateProbeToo() {
        ...
        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "state");
    ...
    public async Task ExecutionDisabledByDefault_RefusesCancel() {
        ...
        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "cancel");
    ...
    public async Task Start_RunsTheDebugStartVerb() {
        var (sut, cli) = CreateSut(execution: true);
    ...
    public async Task Start_WithoutFilePath_IsRefused() {
        ...
        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "start");
    ...
    public async Task EveryMidSessionCommand_MapsToItsDebugVerb(string command) {
        ...
        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, command);
    ...
    public async Task WaitTimeoutSeconds_IsForwarded() {
        ...
        await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "state", waitTimeoutSeconds: 0);
    ...
    public async Task SetBreakpoints_ReplacesTheWholeSet() {
        ...
        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "set-breakpoints",
    ...
    public async Task UnknownCommand_IsRefused() {
        ...
        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "step-sideways");
    ...
    public async Task SessionStillAlive_WarnsToCancel() {
        ...
        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "step-over");
    ...
    public async Task ProfilingOutputDirectory_IsSurfaced() {
        ...
        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "start", filePath: "Main.xaml", profiling: true);
    ...
    public async Task FailureEnvelope_IsReportedAsError() {
        var (sut, _) = CreateSut("""{"Result":"Failure","Message":"No debug session is active."}""", execution: true);

        var result = await sut.ControlDebugSession(CliToolFixtures.ProjectPath, "continue");
    ...
    public async Task UnparseablePayload_ReturnsCliUnparseableResponse() {
        var (sut, _) = CreateSut("not json", execution: true);
    ...
    public async Task MissingProjectJson_IsRefusedBeforeTheExecutionGate() {
        ...
        var sut = new ControlDebugSessionTool(cli, new FakeFilesystemProvider { ProjectJson = null }, CliToolFixtures.Policy(execution: true));
