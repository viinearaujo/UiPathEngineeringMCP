...
public sealed class ValidateActivitySpecTool {
    ...
    [McpServerTool(
        ...
        Title = "Validate Activity Spec",
        ReadOnly = true,
        Destructive = false,
        Idempotent = true),
     Description("Dry-runs a JSON activity spec against the catalog (no file IO). Read resource uipath://authoring/activity-spec for grammar. Next: insert_activities or build_workflow.")]
