...
public sealed class EditWorkflowFileTool {
    ...
    [McpServerTool(UseStructuredContent = true), Description("Replaces an exact string in a .xaml or .cs workflow file inside an existing UiPath project. Fails when oldString is not found or matches multiple locations unless replaceAll is true. Prefer this over write_workflow_file for small changes.")]
