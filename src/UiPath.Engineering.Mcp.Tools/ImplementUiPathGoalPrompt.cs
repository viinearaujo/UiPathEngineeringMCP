...
public sealed class ImplementUiPathGoalPrompt {
    ...
    public static string Render(string projectPath, string goal) =>
        ...
        5. validate_project with build:false and pack:false, then analyze_project_gaps. Remediate resilience, observability, structure, and coded/XAML boundary gaps before done; ignore category=docs. Then update_plan_task to done or blocked. Marking done is not blocked on docs/ADR freshness. Do not call verify_work as the done gate.

        IDIOM: coded [Workflow] entry uses try/catch and Log; XAML only wires REFramework and InvokeWorkflowFile with BCL/framework args. Before done, analyze_project_gaps has no remaining resilience or boundary warnings on files you touched.
