...
public sealed class GetWorkflowDependenciesTool {
    ...
    public async Task<ToolResult> GetWorkflowDependencies(
        ...
        var callers = graph.CallersIndex.TryGetValue(identity, out var incoming)
            ? incoming
            : [];
        return ToolResults.Ok(
            $"Workflow '{identity}': {callers.Count} caller(s), {workflow.InvokeWorkflows.Count} callee(s).",
            new {
                workflow = identity,
